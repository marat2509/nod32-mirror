...
class Mirror
{
    ...
    {
        Log::trace(Language::t('log.running', __METHOD__), static::$version);
    ...
    {
        Log::trace(Language::t('log.running', __METHOD__), static::$version);
        Log::debug(Language::t('mirror.testing_key', static::$key[0], static::$key[1]), static::$version);
        ...
        Log::debug(Language::t('mirror.checking_mirrors', count($test_mirrors), static::$version), static::$version);

        foreach ($test_mirrors as $mirror => $time) {
            Log::trace(Language::t('mirror.mirror_response_time', $mirror, $time), static::$version);
            ...
            if ($version) {
                ...
                Log::debug(Language::t('mirror.remote_version', $version), static::$version);
            } else {
                Log::warning(Language::t('mirror.skipped_unreadable_update_ver', $mirror), static::$version);
                continue;
            }
        }
    ...
    static public function check_mirror($mirror)
    {
        ...
        Log::debug(Language::t('mirror.checking_with_key', $mirror, static::$key[0], static::$key[1]), static::$version);
    ...
    static public function all_channels_up_to_date($mirrorHost, $logDetails = false)
    {
        ...
        foreach (static::$update_variants as $variantKey => $paths) {
            ...
            if ($localVersion !== null) {
                Log::trace(Language::t('mirror.local_version', $localVersion), static::$version);
            }
            if ($remoteVersion !== null) {
                Log::trace(Language::t('mirror.remote_version', $remoteVersion), static::$version);
            }
        ...
        if ($logDetails && !empty($details)) {
            ...
            Log::trace(Language::t('mirror.channel_status', $detailsStr), static::$version, 'all');
        }
    ...
    static public function download_update_ver($mirror, $downloadRandomFile = false, $variantKey = null)
    {
        ...
        if (is_array($headers) and $headers['http_code'] == 200 and $downloaded) {
            if (file_exists($archive) && filesize($archive) === 0) {
                Log::warning(Language::t('mirror.downloaded_empty_update_ver', $mirror), static::$version);
                @unlink($archive);
                return;
            }
            ...
            Log::warning(Language::t('mirror.failed_download_update_ver', $mirror, $headers['http_code'] ?? 'n/a'), static::$version);
            @unlink($archive);
        }
    }
    ...
    static public function download_signature()
    {
        ...
        if ($mirror) {
            Log::info(Language::t('mirror.selected_mirror', $mirrorHost, $mirror['db_version'] ?? 'n/a'), static::$version);
        }
        $total_duration = 0;
        $total_downloaded = 0;
        ...
        if ($processed) {
            ...
            foreach (glob(Tools::ds($web_dir, $version . "-*"), GLOB_ONLYDIR) as $file) {
                ...
                if ($del_files > 0) {
                    static::$updated = true;
                    Log::info(Language::t('mirror.deleted_files', $del_files) . " [" . basename($file) . "]", static::$version);
                }
            }
            ...
            foreach (glob(Tools::ds($web_dir, $version . "-*"), GLOB_ONLYDIR) as $folder) {
                ...
                if ($del_folders > 0) {
                    static::$updated = true;
                    Log::info(Language::t('mirror.deleted_folders', $del_folders) . " [" . basename($folder) . "]", static::$version);
                }
            }
            ...
            Log::warning(Language::t('mirror.update_ver_parse_error', $logHost), static::$version);
        }
    ...
    static protected function process_update_variant($variantKey, $paths, $web_dir, $mirrorHost)
    {
        ...
        Log::debug(Language::t('mirror.processing_variant', $variantKey), static::$version);

        try {
            if (!$mirrorHost) {
                Log::debug(Language::t('mirror.variant_skipped', $variantKey), static::$version);
                return $result;
            }
            ...
            if ($content === false) {
                Log::warning(Language::t('mirror.update_ver_parse_error', $mirrorHost) . " ({$variantKey})", static::$version);
            ...
            if (!empty($download_files)) {
                ...
                Log::debug(Language::t('mirror.no_files_to_download'), static::$version);
            }
            ...
            Log::info(Language::t('mirror.total_size', Tools::bytesToSize1024($total_size)) . " ({$variantKey})", static::$version);

            if ($downloaded > 0 && $duration > 0) {
                $speed = round($downloaded / $duration);
                Log::info(Language::t('mirror.total_downloaded', Tools::bytesToSize1024($downloaded)) . " ({$variantKey})", static::$version);
                Log::info(Language::t('mirror.average_speed', Tools::bytesToSize1024($speed)) . " ({$variantKey})", static::$version);
            }
        ...
        foreach ($chunks as $key => $files)
        {
            ...
            foreach ($handlers as $tmp2)
            {
                ...
                if (is_array($header) and $header['http_code'] == 200 and $header['size_download'] == $expected) {
                    ...
                    Log::info(Language::t('mirror.downloaded_file', $tmp2['mirror']['host'], basename($tmp2['file']['file']),
    ...
    static protected function single_download($download_files, $onlyCheck = false, $checkedMirror = null)
    {
        ...
        foreach ($download_files as $file) {
            foreach ($mirrorList as $id => $mirror) {
                ...
                $time = microtime(true);
                Log::info(Language::t('mirror.downloading_file', $file['file'], $mirror['host']), static::$version);
                $out = Tools::ds($web_dir, $file['file']);
                $dir = dirname($out);
                ...
                if (is_array($header) && $header['http_code'] == 200 && $gotSize == $expectedSize) {
                    ...
                    Log::info(Language::t('mirror.downloaded_file', $mirror['host'], basename($file['file']),
    ...
    static protected function download_files($download_files)
    {
        ...
        Log::info(Language::t('mirror.downloading_files', count($download_files)), static::$version);
        ...
        foreach ($download_files as $file) {
            ...
            if ($expectedSize !== null) {
                ...
                if ($actual !== $expectedSize) {
                    ...
                    @unlink($path);
                    Log::warning(
                        Language::t('mirror.file_size_mismatch', $file['file'], $expectedSize, $actual),
                        static::$version
                    );
                }
            }
    ...
    static public function init($version, $dir)
    {
        ...
        Log::trace(Language::t('mirror.initialized', static::$name), static::$version);
    }
                    ...
                    foreach ($results as $result) {
                        if (Tools::compare_files(@stat($result), $array)) {
                            ...
                            switch ($linkMethod) {
                                case 'hardlink':
                                    link($result, $path);
                                    Log::info(Language::t('mirror.created_hardlink', basename($array['file'])), static::$version);
                                    break;
                                case 'symlink':
                                    symlink($result, $path);
                                    Log::info(Language::t('mirror.created_symlink', basename($array['file'])), static::$version);
                                    break;
                                case 'copy':
                                default:
                                    copy($result, $path);
                                    Log::info(Language::t('mirror.copied_file', basename($array['file'])), static::$version);
                                    break;
                            }
    ...
    static public function filter_files($files)
    {
        ...
        Log::debug(Language::t('mirror.filtered_files', count($filtered_files), count($files)), static::$version);
